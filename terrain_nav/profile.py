...
from .nmea import RadioAltimeterSample
...
def radio_to_ground_profile(
    radio_samples: list[RadioAltimeterSample],
    baro_altitude_m: float | np.ndarray,
) -> GroundProfile:
    if len(radio_samples) < 2:
        raise ValueError("At least two valid radio-altimeter samples are required")
    timestamps = np.asarray([sample.timestamp for sample in radio_samples], dtype=np.float64)
    radio = np.asarray(
        [sample.radio_altitude_m for sample in radio_samples], dtype=np.float64
    )
...
def remove_outliers_hampel(
    ...
    for index in range(values.size):
        ...
        mad = scale * np.nanmedian(np.abs(window - median))
        if np.isfinite(values[index]) and mad > 1e-9:
            if abs(values[index] - median) > n_sigma * mad:
                result[index] = median
    return result
