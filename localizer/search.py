...
class TerrainNavigator:
    ...
    def localize(
        ...
        *,
        ...
        filtered = remove_outliers_hampel(
            profile.heights_m,
            window_size=self.config.hampel_window_size,
            n_sigma=self.config.hampel_n_sigma,
            passes=self.config.hampel_passes,
        )
        processed = smooth_moving_average(filtered, window_size=3)
        ...
        for window_index, (begin, end) in enumerate(windows):
            ...
            previous_confidence, numeric = confidence_level(
                ...
                minimum_score_gap=self.config.confidence_min_score_gap,
            )
            for item in active:
    ...
    def _expand_parent(
        ...
        *,
        ...
        if initial:
            ...
            global_speed_min = max(
                0.5,
                initial_speed_mps * self.config.global_speed_min_multiplier,
            )
            global_speed_max = max(
                global_speed_min,
                initial_speed_mps * self.config.global_speed_max_multiplier,
            )
            speeds = _inclusive_range(
                max(global_speed_min, parent.ground_speed_mps - speed_delta),
                min(global_speed_max, parent.ground_speed_mps + speed_delta),
                self.config.speed_step_mps,
            )
        ...
        raw_scores, valid_fraction = fast_scores_batch(
            measured,
            heights,
            absolute_height_sigma_m=self.config.absolute_height_sigma_m,
        )
